...
def _make_local_manifest_path(build_dir: Path, job_name: str) -> Path:
    return build_dir / f"jobs/{kebabcase(job_name).lower()}/job-manifest.json"
...
def _write_job_manifest(build_dir: Path, manifest: JobManifest) -> Path:
    local_manifest_path = _make_local_manifest_path(build_dir, manifest.job_config.name)
    ...
    with open(local_manifest_path, "w") as fout:
        ...
        json_dump = json.dumps(remove_nones(json_dict),
                               allow_nan=False,
...
class JobService(BaseService):
    ...
    def push(self, manifests: List[Tuple[Path, JobManifest]], local: bool = False, **kwargs) -> List[str]:
        ...
        for manifest_path, manifest in manifests:
            ...
            with Spinner(text=f"Pushing job {manifest.job_config.name}") as s:
                ...
                target_manifest_path = str(manifest_path).replace(manifest_dir, deployment_dir)

                s.info(f"Uploading wanna job manifest to {target_manifest_path}")
                with open(target_manifest_path, "w") as f:
                    f.write(manifest.json())

                results.append(target_manifest_path)
    ...
    def run(jobs: List[str],
        ...
        for manifest_path in jobs:
            ...
            # TODO: Sets Alerts on failed job ???
            manifest.job_payload['project'] = gcp_profile.project_id or manifest.job_payload['project']
            manifest.job_payload['location'] = gcp_profile.region or manifest.job_payload['location']
            if manifest.job_type is CustomJobType.CustomContainerTrainingJob:
                ...
                training_job = CustomPythonPackageTrainingJob(**manifest.job_payload)
            else:
                training_job = None
            ...
            if training_job:
                with Spinner(text=f"Initiating {manifest.job_config.name} custom job") as s:
                    ...
                    training_job.run(
                        ...
                        service_account=gcp_profile.service_account or manifest.job_config.service_account,
                ...
                if sync:
                    ...
                    with Spinner(text=f"Running custom job {manifest.job_config.name} in asyn sync mode") as s:
                        s.info(
                            f"Job Dashboard in "
                            f"https://console.cloud.google.com/vertex-ai/locations/{manifest.job_config.region}/training/{job_id}?project={manifest.job_config.project_id}"  # noqa
                        )
        ...
        if isinstance(instance, TrainingCustomJobModel):
            ...
            return CustomJobManifest(
                ...
                job_payload={
                    ...
                    "worker_pool_specs": [MessageToDict(s._pb) for s in list(worker_pool_specs)],
