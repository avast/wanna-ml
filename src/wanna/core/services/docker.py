...
class DockerService:
    ...
    def _build_image(
        ...
        if should_build and not self.quick_mode:
            if self.cloud_build:
                logger.user_info(f"Building {docker_image_ref} docker image in GCP Cloud build")
                ...
                logger.user_info(f"Building {docker_image_ref} docker image locally with {build_args}")
                image = docker.build(context_dir, file=file_path, load=True, tags=tags, **build_args)
                self._write_context_dir_checksum(self.build_dir / docker_image_ref, context_dir)
                return image  # type: ignore
        else:
            logger.user_info(
                f"Skipping build for context_dir={context_dir}, dockerfile={file_path} and image {tags[0]}"
            )
            return None
    ...
    def _pull_image(self, image_url: str) -> Union[Image, None]:
        if self.cloud_build or self.quick_mode:
            ...
            with logger.user_spinner(text="Pulling image locally"):
    ...
    def push_image(self, image_or_tags: Union[Image, List[str]], quiet: bool = False) -> None:
        ...
        if not self.cloud_build:
            ...
            with logger.user_spinner(text=f"Pushing docker image {tags}"):
