...
def mock_zones_client():
    with mock.patch("google.cloud.compute_v1.ZonesClient", mocks.MockZonesClient) as _fixture:
        yield _fixture


@pytest.fixture(scope="session", autouse=True)
def mock_regions_client():
    with mock.patch("google.cloud.compute_v1.RegionsClient", mocks.MockRegionsClient) as _fixture:
        yield _fixture


@pytest.fixture(scope="session", autouse=True)
def mock_images_client():
    with mock.patch("google.cloud.compute_v1.ImagesClient", mocks.MockImagesClient) as _fixture:
        yield _fixture
...
def mock_machine_types_client():
    with mock.patch(
        "google.cloud.compute_v1.MachineTypesClient", mocks.MockMachineTypesClient
    ) as _fixture:
        yield _fixture
...
def mock_storage_client():
    with mock.patch("google.cloud.storage.Client", mocks.MockStorageClient) as _fixture:
        yield _fixture
