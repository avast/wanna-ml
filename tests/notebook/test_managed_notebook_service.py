...
class TestManagedNotebookService(unittest.TestCase):
    ...
    def test_list_running_instances(self):
        config = load_config_from_yaml(
            "samples/notebook/managed-notebook/wanna.yaml", "default"
        )
        nb_service = ManagedNotebookService(config=config, workdir=Path("."))
        running_notebooks = nb_service._list_running_instances(
            project_id=self.project_id, location=self.region
        )
        assert (
            f"projects/{self.project_id}/locations/{self.region}/runtimes/minimum-setup"
            in running_notebooks
        )
        assert (
            f"projects/{self.project_id}/locations/{self.region}/runtimes/maximum-setup"
            in running_notebooks
        )
        assert (
            f"projects/{self.project_id}/locations/{self.region}/runtimes/xyz"
        ...
        expected_to_be_created = ManagedNotebookModel.parse_obj(
            {
                ...
                "owner": "jacek.hebda@avast.com",
                "kernels": [
                    "gcr.io/projectId/imageName1",
                    "gcr.io/projectId/imageName2",
                ],
            }
        )
