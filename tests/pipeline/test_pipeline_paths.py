...
class TestPipelineService(unittest.TestCase):
    ...
    def test_gcs_paths(self) -> None:
        json_spec_file = self.pipeline_paths.get_gcs_pipeline_json_spec_path(
            self.version
        )
        wanna_manifest_file = self.pipeline_paths.get_gcs_wanna_manifest_path(
            self.version
        )
        ...
        self.assertEqual(
            json_spec_file,
            "gs://test_bucket/wanna-pipelines/sklearn/deployment/test/manifests/pipeline-spec.json",
        )
        self.assertEqual(
            wanna_manifest_file,
        ...
        self.assertEqual(
            pipeline_root, "gs://test_bucket/wanna-pipelines/sklearn/executions/"
        )

    def test_local_paths(self) -> None:
        json_spec_file = self.pipeline_paths.get_local_pipeline_json_spec_path(
            self.version
        )
        wanna_manifest_file = self.pipeline_paths.get_local_wanna_manifest_path(
            self.version
        )
        ...
        self.assertEqual(
            pipeline_root,
            str(self.pipeline_build_dir / "wanna-pipelines/sklearn/executions"),
        )
